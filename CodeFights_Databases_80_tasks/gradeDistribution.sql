...
BEGIN
    SELECT marks.name as Name, marks.id as ID
    FROM (
        SELECT name, id,
            midterm1 * 0.25 + midterm2 * 0.25 + final * 0.5 as o1,
            midterm1 * 0.5 + midterm2 * 0.5 as o2,
            final as o3
        FROM Grades
    ) marks
    WHERE marks.o3 > marks.o2 AND marks.o3 > marks.o1
